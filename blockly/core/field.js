...
Blockly.Field.prototype.init = function () {
  ...
  console.log("Field.prototype.init.this:", this);

  // 블록의 두께 정의_2 - Define the thickness of the block _2
  // NowCodingBlockType 드랍다운 위치 정의한다 - Define NowCodingBlockType drop-down location
  /**
   * NowCoding Field positioning.
   * TODO[25/07/19]: Lee Middlebrook - Move and uncouple from Field / Blockly
   * TODO[25/07/19]: Lee Middlebrook - Convert literals to CONSTANTS... 
   **/
  let textRenderY = this.size_.height / 2;
  let rectRenderY = 0;
  if (this.sourceBlock_) {
    if (this.sourceBlock_.NowCodingBlockType === "TextLabelInputHole") {
      textRenderY += 9;
      rectRenderY += 2;
    } else if (this.sourceBlock_.NowCodingBlockType === "TextLabelInputHoleLoop") {
      textRenderY += 10;
      rectRenderY += 6;
    } else if (this.sourceBlock_.NowCodingBlockType === "Dropdown") {
      textRenderY += 4;
    } else if (this.sourceBlock_.NowCodingBlockType === "TextLabelDropdown") {
      textRenderY += 7;
      rectRenderY += 2;
      ...
      // textRenderY += 10;
      rectRenderY -= 4;
    } else if (this.sourceBlock_.NowCodingBlockType === "LogicDefaultNoMargin") {
      // textRenderY -= 3;
      // rectRenderY -= 3;
  ...
  if (this.sourceBlock_.NowCodingBlockType === "MathDefault") {
    // console.log("##################", this)
    ...
    this.textElement_ = Blockly.utils.createSvgElement('text',
      { 'class': 'blocklyText 22223333 NOWCODING_MIN_INPUT_NUMBER_WIDTH', 'y': textRenderY + 4, 'x': Blockly.BlockSvg.NOWCODING_MIN_INPUT_NUMBER_WIDTH / 2 + 2 },
      this.fieldGroup_);

  } else if (this.sourceBlock_.NowCodingBlockType === "MathWithDropdown") {
      ...
      this.borderRect_ = Blockly.utils.createSvgElement('rect',
        {
          'rx': 2,
          'ry': 2,
          'x': - 2 - Blockly.BlockSvg.SEP_SPACE_X / 2,
          'y': rectRenderY,
          'height': 25
        }, this.fieldGroup_);
    ...
    /** @type {!Element} */
    this.textElement_ = Blockly.utils.createSvgElement('text',
      { 'class': 'blocklyText NOWCODING_MIN_INPUT_NUMBER_WIDTH', 'y': textRenderY + 4, 'x': Blockly.BlockSvg.NOWCODING_MIN_INPUT_NUMBER_WIDTH / 2 },
      this.fieldGroup_);
    ...
    this.textElement_ = Blockly.utils.createSvgElement('text',
      {
          'class': 'blocklyText sdlkfsdlkfjlk',
          'y': textRenderY,
          'x': Blockly.BlockSvg.SEP_SPACE_X / 2,
          'height': 90
      },
  ...
  //변수이거나, 드랍다운일 경우 - If it is a variable or a dropdown
  if ((this.menuGenerator_ && this.menuGenerator_.length > 0) || this.variable_) {
 ...
 */
Blockly.Field.prototype.updateEditable = function (isFieldDropdown) {

  let className = "blocklyEditableText";
  if (isFieldDropdown != null && isFieldDropdown != undefined && isFieldDropdown) {
...
Blockly.Field.prototype.updateWidth = function () {
  ...
  if (this.borderRect_) {
    ...
    //TODO[16/07/19]: Lee Middlebrook - whats happening here, separate from CORE
    if (this.sourceBlock_.NowCodingBlockType === "MathDefault") {
      this.borderRect_.setAttribute('width',
        width + Blockly.BlockSvg.SEP_SPACE_X + Blockly.BlockSvg.NOWCODING_MIN_INPUT_NUMBER_WIDTH);
    } else {
      this.borderRect_.setAttribute('width',
