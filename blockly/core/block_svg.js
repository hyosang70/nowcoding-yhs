...
Blockly.BlockSvg = function (workspace, prototypeName, opt_id) {
  ...
  this.svgPathDark_ = Blockly.utils.createSvgElement('path',
    { 'class': 'blocklyPathDark', 'transform': 'translate(0,2) scale(1,1)' },
    this.svgGroup_);
...
/**
 * 블록 타입이 컨트롤 블록인지 체크 - block type is control blocks check
 */
Blockly.BlockSvg.prototype.isControlBlock = false;

/**
 * 블록 타입을 컨트롤 블록으로 정의 - Define block type as control block
 */
Blockly.BlockSvg.prototype.setControlBlock = function () {
...
Blockly.BlockSvg.prototype.updateMovable = function () {
  ...
  if (this.NowCodingBlockType == "Dropdown") {
    Blockly.utils.addClass(
        /** @type {!Element} */(this.svgGroup_), 'NOWCODING_DROPDOWN');
  }
  if (this.NowCodingBlockType == "MathDefault") {
        Blockly.utils.addClass(
          /** @type {!Element} */(this.svgGroup_), 'NOWCODING_MATHNUM');
    }
  if (this.isShadow_) {
    Blockly.utils.addClass(
      /** @type {!Element} */(this.svgGroup_), 'NOWCODING_SHADOW');

    if (this.NowCodingBlockType === "MathDefault") {
      Blockly.utils.addClass(
          /** @type {!Element} */(this.svgGroup_), 'NOWCODING_MATHDEFAULT');
    }
  }
  if (this.isMovable()) {
