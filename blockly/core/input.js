...
goog.require('Blockly.ControlIcon');
...
Blockly.Input = function (type, name, block, connection) {
  ...
  /**
   * * 블록 타입이 컨트롤 블록일 경우 Input Icon을 추가
   * Add Input Icon when block type is control block
   * TODO make this an .append function applied in the block definition
   */
  if (this.sourceBlock_.isControlBlock) {
    if (name === "ELSE") { // Add Remove button to ELSE statement
      this.controlButton = new Blockly.ControlIcon(BUTTON_OPERATION_TYPE.REMOVE_ELSE, this.sourceBlock_, this.name, this, this.sourceBlock_.getHeightWidth());
    }

    if (type == Blockly.INPUT_VALUE ) {
      if (this.name.indexOf("IF") != -1 && this.name != "IF0") {
        this.controlButton = new Blockly.ControlIcon(BUTTON_OPERATION_TYPE.REMOVE_ELSE_IF, this.sourceBlock_, this.name, this, this.sourceBlock_.getHeightWidth());
      }
    }
  }
...
Blockly.Input.prototype.setNowCodingBlockType = function (blockType) {
  this.NowCodingBlockType = blockType;
  // return this;
};
